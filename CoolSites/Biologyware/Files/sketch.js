...
function loadFile() {
  ...
  if (e.target.files[0]) {	handleScore(e.target.files[0])
...
function write(strong, xps, yps)
{
	//var temp = ctx.fillStyle
	//ctx.fillStyle = text_color
	ctx.fillText(strong, xps*zoomNum, yps*zoomNum)
	//ctx.fillStyle = temp
	ctx.lineWidth = stroke_width * ctx.font.replace(/\D/g,'') / 128;
	ctx.strokeText(strong, xps*zoomNum, yps*zoomNum)
}

function textSize(size)
{
	ctx.font = (size*zoomNum) + "px serif"
}

let debugText = ""

function fill(color, g=undefined, b=undefined, a=undefined)
{
	if(g === undefined)
	{
		ctx.fillStyle = color
	...
	else
	{
		...
		{
			ctx.fillStyle = "rgba("+color+","+g+","+b+","+a/255+")"
		}
	}
	//text_color = color
}
...
function strokeWeight(size){
	ctx.lineWidth = size
	stroke_width = size*zoomNum
}

function drawSprite(img, dx, dy, dWidth=undefined, dHeight=undefined, sx=undefined, sy=undefined, sWidth=undefined, sHeight=undefined) {
	if(dWidth === undefined)
...
function loop(){do_draw = true}
function noLoop(){do_draw = false}

let frameCount = 0

function square(x, u, s)
{
	ctx.fillRect(x, u, s, s)
	ctx.strokeRect(x, u, s, s)
}

/*let table_to_save = []
let seen_trans = []*/

function thingRotate(amount)
{
	ctx.rotate(amount * Math.PI / 180)
}
	/*if(add_to)
	{
		seen_trans.push([amount, "rotate"])
	}
}
function thingTranslate(amount, add_to=true)
{
	ctx.rotate(amount * Math.PI / 180)
	if(add_to)
	{
...
{
	table_to_save = []
	table_to_save.push(ctx.fillStyle, ctx.strokeStyle, stroke_width, ctx.font, ctx.textAlign, ctx.textBaseline)
...
frameCount++
  ...
  if (microGamePicked == -1) {
    ...
    write("Biologyware", width / (2 * zoomNum), height / (3 * zoomNum));
    ...
    if (/*getTargetFrameRate() - frameRate() > 10 && okayitsgood*/ true == false) {
      background("black");
      write("psst, wait a bit", width / (2 * zoomNum), height / (3 * zoomNum));
      write(
    ...
    if (microGamePicked != 0) {
      ...
      strokeWeight(2);
      if (boss == false) {
        ctx.fillRect(10, 380, (timer / microGameTimes[microGamePicked - 1]) * 380, 10);
        ctx.strokeRect(10, 380, (timer / microGameTimes[microGamePicked - 1]) * 380, 10);
      }
    }
    ...
    if (calmTimer > 0 || startRunningIt > 0) {
      ...
      if (calmTimer > 0) {
        textSize(30);
        fill("white");
        write(textToSay, width / (2 * zoomNum), 30);
      }
      if (calmTimer <= 0 && startRunningIt > 0) {
      ...
      if (lives == 0) {
        ...
        textSize(64);
        strokeWeight(2);
        write("GAME OVER", width / (2 * zoomNum), 70);
        textSize(32);
        write(
        ...
        if (nameEnterScreen == true) {
          ...
          textSize(24);
          write(nameToPutIn, width / (2 * zoomNum), height / (2 * zoomNum));
          if (pressedKey == "EnterPressed") {
		...
		download("HighScoreInfo.txt", saveInfo)
  ...
  if (paused == true) {
    fill(0, 0, 0, 100);
    ctx.fillRect(-10, -10, width * (2 * zoomNum), width * (2 * zoomNum));
	fill("white")
    write("Paused", width / (2 * zoomNum), height / (2 * zoomNum));
  }
  if (paused == "nah") {
    paused = false;
  }
	write(debugText, 50, 50)
...
function draw4() {
  ...
  drawSprite(DNAFloor, DNAChainX - 800, 400 - 64, 1600, 64);
  drawSprite(DNAPlayer, DNAX, DNAY, 18 * 2, 17 * 2);
}
...
function draw2() {
  ...
  drawSprite(Bottle, 300, 270);
  drawSprite(Cork, game2DrawCarX, 320);
}
  ...
  for (i = 0; i <= 2; i++) {
    ...
    if (EnzymeRotation[i] == 1 || EnzymeRotation[i] == 3) {
      ...
      if (EnzymeRotation[i] == 2) {
        ctx.translate((10 + 120 * i + 50) * 2, (260 + 50) * 2);
      }
    }
    thingRotate(EnzymeRotation[i] * 90);
    drawSprite(eval("Enzyme" + EnzymeOrder[i]), 10 + 120 * i, 260, 100, 100);
    ctx.resetTransform()
  }
  fill(0, 0, 0, 3);
  outline("black");
  ...
  if (selection3 == -1) {
    ...
    if (EnzymeRotation[selection3] == 1 || EnzymeRotation[selection3] == 3) {
      ...
      if (EnzymeRotation[selection3] == 2) {
        ctx.translate((10 + 120 * selection3 + 50) * 2, (260 + 50) * 2);
      }
    }
    thingRotate(EnzymeRotation[selection3] * 90);
    drawSprite(eval("Protein" + ProteinType), 10 + 120 * selection3, 260, 100, 100);
    ctx.resetTransform()
  }
}
...
function draw7() {
  ...
  drawSprite(game7Passport, 200, 0);
  strokeWeight(0);
  textSize(20);
  fill("black");
  if (whatIsWrong == 1) {
  ...
  if (whatIsWrong == 2) {
    write("Purple", 305, 160);
  } else {
    write("Green", 305, 160);
  }
  if (whatIsWrong == 3) {
    write("Tieless", 315, 240);
  } else {
    write("Tie", 295, 240);
  }
  ...
  if (winning == true) {
    drawSprite(eval("game7ProteinGuy" + whatIsWrong), 0, 0);
  } else {
    drawSprite(game7ProteinGuy0, 0, 0);
  }
  fill(0, 0, 0, 0);
  outline("red");
  strokeWeight(2);
...
function draw9() {
  ...
  strokeWeight(1);
  if (textDump == "") {
    write("(Type to answer.)", width / (2 * zoomNum), height / (1.5 * zoomNum));
  } else {
    write(textDump, width / (2 * zoomNum), height / (1.5 * zoomNum));
  }
}
...
setInterval(draw_main, 1000/(50/3)/4)
